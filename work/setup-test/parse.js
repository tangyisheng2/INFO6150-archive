...
const structureCheck = root.querySelectorAll('ul.people > li > span');
...
const peopleEl = root.querySelectorAll('ul.people');
if( !peopleEl[0] ) {
  console.error("ERROR: You are missing the class of people");
  process.exit(1);
}
if( peopleEl.length !== 1 ) {
  console.error("ERROR: You have too many elements with the class of people");
  process.exit(1);
}
...
parts.forEach( part => {
  elements[part] = root.querySelectorAll(`ul.people .${part}`);
  if(!elements[part][0]) {
